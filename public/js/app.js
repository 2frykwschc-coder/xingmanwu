...
function navigate(p){document.querySelectorAll('.page').forEach(e=>e.classList.remove('active'));const m={'home':'ph','browse':'pb','collections':'pc','stats':'ps','tools':'pt'};$(m[p]||'ph').classList.add('active');st.page=p;window.scrollTo({top:0});if(p==='home')loadHome();if(p==='browse')loadBrowse();if(p==='collections')loadCollections();if(p==='stats')loadStats();if(p==='tools')loadTools();}
...
async function loadTools(){const c=$('tc');try{const z=await api('/api/zero-chapters?limit=1'),a=await api('/api/stats');const totalz=z.total||0;c.innerHTML='<h3>🔧 查源工具</h3><p>使用浏览器绕过 Cloudflare 自动查找零章漫画的替代阅读源。</p><hr style="margin:16px 0;border-color:rgba(255,255,255,.06)"><div style="background:var(--card);border-radius:12px;padding:20px;margin-bottom:16px"><h4>📊 当前状态</h4><div style=margin-top:12px><div>漫画总数: '+(a.totalManga||0).toLocaleString()+' 部</div><div>零章: '+totalz.toLocaleString()+' 部</div><div>已通过脚本找到: <span id=altCount style=color:var(--accent)></span></div></div></div><hr style="margin:16px 0;border-color:rgba(255,255,255,.06)"><h4>📝 使用方法</h4><ol style="padding-left:20px;margin-top:12px;display:flex;flex-direction:column;gap:8px"><li>打开任意零章漫画的<b>详情页</b>（点进去看就行）</li><li>按 <kbd style="background:var(--card-hover);padding:2px 8px;border-radius:4px">F12</kbd> 打开<b>开发者工具</b></li><li>点顶部 <kbd style="background:var(--card-hover);padding:2px 8px;border-radius:4px">Console</kbd>（控制台）</li><li>把下面的代码<b>全部复制</b>，粘贴到控制台，按回车</li><li>脚本自动运行，让它挂后台就行</li></ol><br><div style="background:#1a1a2e;border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:16px"><pre style="font-size:12px;line-height:1.5;overflow:auto;white-space:pre-wrap;word-break:break-all" id=scriptCode></pre><button class="btn btn-primary btn-sm" style="margin-top:12px" onclick="copyScript()">📋 复制代码</button></div><br><p style="color:var(--text-dim);font-size:13px">⏱️ 每部约 2 秒，2,000 部约 1 小时。跑完零章数量会逐渐减少。</p>';try{const ac=await api('/api/fix-alt-summary');$('altCount').textContent=ac.count+' 部'}catch{$('altCount').textContent='0 部'}fetchScriptCode()}catch{c.innerHTML='<div style="text-align:center;padding:40px;color:var(--text-dim)">⏳</div>'}}

function fetchScriptCode(){fetch('/fix_alt_browser.js?'+Date.now()).then(r=>r.text()).then(code=>{$('scriptCode').textContent=code}).catch(()=>{$('scriptCode').textContent='// 未能加载脚本代码，请直接查看 /fix_alt_browser.js 文件'})}

function copyScript(){const code=$('scriptCode').textContent;navigator.clipboard.writeText(code).then(()=>{const btn=event.target;const ot=btn.textContent;btn.textContent='✅ 已复制!';setTimeout(()=>btn.textContent=ot,2000)}).catch(()=>{const ta=document.createElement('textarea');ta.value=code;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta)})}
